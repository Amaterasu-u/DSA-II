...
public:
    ...
    void processMin() {
        ...
        int minIndex = size_ / 2 + 1;
        for (int i = (size_ / 2 + 1) + 1; i <= size_; i++) {
            if (heap[i].amount == minAmount && heap[i].timestamp < heap[minIndex].timestamp) {
                minIndex = i;
            }
        }
    ...
    void heapSort() {
        ...
        // Restoring the original size after sorting
        size_ = originalSize;
    }
};
...
int main() {
    ...
    while (1) {
        ...
        cout << "9. Heap Sort Bills" << endl; // New option
        cout << "10. CLOSE THE RESTAURANT" << endl;
        ...
        if (choice == 1) {
            ...
            pq.heapSort();
            cout << "Bills sorted by amount." << endl;
        } else if (choice == 10) {
